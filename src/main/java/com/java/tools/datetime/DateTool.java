...
public class DateTool {
	...
	public static void main(String[] args) {
		...
		String currentTime = date2String("yyyy-MM-dd HH:mm:ss");
		System.out.println("Current TimeZone : " + currentTimezone);
		System.out.println("Current time for [" + currentTimezone + "] is " + currentTime);

		// 显示世界每个时区当前的实际时间
		String[] ids = fecthAllTimeZoneIds();
		for (int i = 0; i < ids.length; i++) {
			System.out.println("   [" +  ids[i] + "]     " + string2TimezoneDefault(currentTime, ids[i]));
		}
	}
	...
	public static String[] fecthAllTimeZoneIds() {
		...
		for (int i = 0; i < ids.length; i++) {
			v.add(ids[i]);
			//System.out.println("Add Timezone [" + ids[i] + "]");
		}
